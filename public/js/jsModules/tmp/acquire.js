...
const
	...
	var
		...
		moduleMaker = function moduleMaker(id) {
			$this = this;
			if(isAcquired(id)) return modules[id];
	...
	// don't want outsider redefining "acquire" and don't want
	// to use arguments.callee so name the function here.
	let acquire = function (name) {
		if(!isAcquired(name))
	...
	acquire.moduleMaker = moduleMaker;
	
	return acquire;
})();

const moduleMaker = acquire.moduleMaker;

var run = function (name) {
	acquire(name); // doesn't return exports
};
...
export { run, acquire, getExportedData };
