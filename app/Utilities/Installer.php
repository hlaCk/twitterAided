...
class Installer
{
    ...
    public static function createDbTables($host, $port, $database, $username, $password, $resetDB = false)
    {
        ...
        static::saveDbVariables($host, $port, $database, $username, $password, env('DB_PREFIX', null));
    ...
    {
        $prefix = !$prefix ? strtolower(str_random(3) . '_') : $prefix;
    ...
    public static function createCompany($name, $email, $locale)
    {
        ...
        setting()->set([
            ...
            'general.default_currency'      => 'SAR',
            'general.default_locale'        => $locale,
        ]);
        setting()->save();
    }

    public static function createUser($email, $password, $locale, $roleID = '1')
    {
        // Create the user
        $user = User::create([
